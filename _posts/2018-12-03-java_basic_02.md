...
Ball b1 = 레퍼런스 변수 선언. 
<textarea>new : 인스턴스 생성을 위한 연산자
new Ball() --> Ball 인스턴스(객체)를 메모리에 생성한다.
레퍼런스 변수명이 다를경우 같은 인스턴스를 생성했더라도 실제로는 다른 인스턴스이다.</textArea>
TV 설계도 = 클래스
...
객체의 응집도 : 관련된것이 한곳에 모여있는것을 말한다.
객체의 추상화 : 중요한것은 남기고 불필요한것은 없앤다. 
...
class Calulator extends Cal2 { // extends는 상속을 뜻한다.
...
public class MemberTest {
    public static void main(String[] args){
        ...
        Member M2 = new Member("베리굿"); // 생성자 오버로딩 : 패러미터를 받는값이 다를경우 같은 이름의 
        여러개의 생성자가 작성이 가능하다.
